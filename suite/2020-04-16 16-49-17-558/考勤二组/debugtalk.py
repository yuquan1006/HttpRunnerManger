...
def get_current_timestamp(str_len=13, day=0):
    """ get current timeStamp, default format is second（9）/millisecond（13）
    """
    unix_time = time.mktime((datetime.datetime.now()+datetime.timedelta(days=day)).timetuple())
    print(unix_time)
    if str_len == 13:
...
def get_endtimestamp(value,hour=0.1):
    '''
    根据时间戳取几个小时后时间戳
    :param value:时间戳
    :param hour:int 几个小时
...
'''ailie2020-04-09'''   
#获取当前时间的时间戳，如果是申请的按上午下午就只需要传两个相同的时间戳
def getUnixTimestampOfNow():
    nowT = time.time()
    return str(int(nowT)) + "000"


#获取昨天开始的时间戳，如果是申请的按天就需要传两个不相同的时间戳，昨天开始的时间戳HR昨天结束的时间戳
def getUnixTimestampOfYST():
    # 获取当前日期
    ...
    # 获取昨天开始的时间戳
    yesterday_start_time = str(int(time.mktime(time.strptime(str(yesterday_date), "%Y-%m-%d"))))+ "000"
...
def getUnixTimestampOfYET():
    # 获取当前日期
    today_date = datetime.date.today()
    
    # 获取昨天开始的时间戳
    yesterday_end_time = str(int(time.mktime(time.strptime(str(today_date), "%Y-%m-%d"))) - 1)  + "000"
