    ...
    def load_testsets_by_path(path):
        ...
        if os.path.isdir(path):
            ...
            try:
                ...
                if testset["testcases"] or testset.get("api"):
...
class TestcaseParser(object):
    ...
    def eval_content_with_bindings(self, content):
        ...
        if isinstance(content, basestring):
            ...
            #content = self._eval_content_variables(content) # 初始
            # 增加判断，如果content为字符串且长度大于50认为不需要替换变量
            if isinstance(content,str): # 是变量
                if len(content)<200:  # 长度小于100认为，用绑定值替换变量
